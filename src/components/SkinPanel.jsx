            ...
            <div>
                ...
                return(
                    <div key={skin.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px", border: `1px solid ${activa ? "#000" : "#ccc"}`, padding: "12px", width: "140px", background: activa ? "#f0f0f0" : "#fff" }}>
                        <img src={`/${skin.archivo}_normal.png`} alt={skin.nombre} style={{ width: "80px", height: "80px", objectFit: "contain", opacity: desbloq ? 1 : 0.4 }}/>
                        ...
                        <button onClick={() => desbloq && onChangeSkin(skin.id, skin.archivo)} disabled={!desbloq||activa} style={{ background: "#f0f0f0", border: "1px solid #999", padding: "3px 10px", fontFamily: "monospace", fontSize: "11px", cursor: desbloq && !activa ? "pointer" : "default", color: activa ? "#555" : "#000", width: "100%" }}>{activa ? "Activa" : desbloq ? "Usar" : "Bloqueado"}</button>
                    </div>
                )
            })}
                    </div>
